...
# ----------------- SET PHYSICAL PARAMETERS -----------------
    ...
    # MPC PARAMETERS
    Nrt = 10 #15                        # number of round trips
    Npass = 5 #2*Nrt                # number of passes
    ...
    pres = 3.0 #1.5 #1.5 #10e-10 #1.5    # gas pressure [bar]
    gas = :Xe #:Kr                       # gas type
    ...
    if nl_mode_matching == true
        ...
        w0_NL = sqrt(M2*λ0*L * sqrt(σ*(2*R/L -1)) /(2*pi))   #123.62e-6 #125e-6       # mode-matched waist for nonlinear regime

        # # Nonlinear backpropagate from focus to mirror 
        # M_mirr = [1 0;-2/R 1];
        # M2_prop = [1 L/2;0 1];
        # Mtot = M2_prop*M_mirr;
        # A = Mtot[1,1];
        ...
        # wm = sqrt(sqrt(σ)*λ0/pi * 1/imag(1/q_mNL))  # beam waist on the mirror

        # # Linearly forward propagate from mirror to focus
        # M_mirr = [1 0;-2/R 1];
        # M2_prop = [1 L/2;0 1];
        # Mtot = M_mirr*M2_prop;
        # A = Mtot[1,1];
        ...
        # q_m = 1/(n0/-R + 1im*λ0/(pi*wm^2))
        # q_0L = (A*q_m + B)/(C*q_m + D)
        # w0 = sqrt(λ0/pi * 1/imag(1/q_0L))
        ...
        #w0 = sqrt(M2*R*λ0 * sqrt(C*(2-C)) /(2*pi))         # ! this value is used to define the gaussian beam in the beginning of the simulation
        w0 = sqrt(M2*λ0*L * sqrt(2*R/L -1) /(2*pi))   #123.62e-6 #125e-6       # these two eq. for w0 should give the same value                                         

        # beam waist on the mirror
        # wm = sqrt(M2*R*λ0 * sqrt(C/(2-C)) /pi) 
        wm = 546.9e-6 #w0*sqrt(2*R/(2*R-L))
    end
...
# ----------------- SET SIMULATION GRID ----------------------------
    ...
    R_Hankel = 2 #3.0*wm           # aperture radius for Hankel transform (assume field ≈0 for r>R ) [m]  
...
# ----------------- DEFINE INPUT VIA COMPLEX FROG SPECTRUM -----------------
    ...
    Etr = Fields.GaussGauss(grid.t, q.r, τ, 1, wm)
    Etr .*= cos.(PhysData.wlfreq(λ0).*grid.t)
    Eωr = FFTW.rfft(Etr, 1)
    ...
    # calculate kz(ω, k_perp)
        ...
        φr_foc = -kz .* reshape(q.r.^2/(2*R), (1, length(q.r)))
    ...
    if use_FROG==true
        ...
        # CREATE DATAFIELD (taken from https://github.com/LupoLab/Luna.jl/issues/357)
            ...
            Et = sqrt.(Maths.gauss.(grid.t, fwhm=350e-15))
            # w0 = 518.8960331584037e-6 #555.4198741358597e-6
            ...
            # Eωk = q * Eωr_foc                               # transform to reciprocal space
